...
const Login = (props) => {
    console.log("%cConnectionContent -- ContentArea -- Login", 'background: lightgreen; color: black;')
    const { login } = useAuth()
    ...
    function displayErrorBox(passwordError = false, serverRes = ""){
        ...
        serverRes === "" ? setPasswordMsg(<span>Vous devez remplir les champs pour vous connecter.</span>) : setPasswordMsg(serverRes)
    }
    ...
    const handleClickSubmit = () => {         
        function displayErrorsAndLoader() {
            ...
            if(nameError || passwordError){
                ...
                login(nameRef.current.value.trim(), passwordRef.current.value.trim())
                .then(() => {
                    displayValidBox()
                })
                .catch((error) => {
    ...
    return(
        <form className="wrapComponent" action="/forums/" method='post' noValidate="novalidate" data-status="init">
            <Button css="btnSite w-full  mb-2" function={()=>props.setMode("inscription")}>S'inscrire</Button>
            ...
            <Button css="btnSite w-full" function={()=>handleClickSubmit()}>Connexion</Button>
